...
Despite tremendous increases in available numerical computational power in the latter half of the previous-, and the early parts of the current century, any such approximate scheme used is still heavily limited w.r.t.\ the system size. In practice, most methods are limited to systems of containing on the order of $10^3$ electrons [citation needed]. Extracting information from larger systems neccessitate the use of classical or semi-classical algorithms, such as molecular dynamics (MD). Using MD, the time evolution of up to around $10^7$ particles can feasibly be simulated over the order of nano seconds \cite{Zhao2013,Reddy}. As macroscopic objects contain on the order of $10^{23}$ atoms, we are ostensibly a long way away from even classically modelling the constituent particles directly. In this region, continuum models are used.
