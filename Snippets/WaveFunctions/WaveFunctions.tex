...
\end{align}
We note that the overall function shape of STO-3G already closely resembles the actual STO. At higher values of n, they become indistinguishable without extremely close inspection. From the insets of \fig{sto24g} we note that the sum of Gaussians always satisfy $\mathrm{d}/\mathrm{d}r \sum_k g_k^\alpha(r)\big|_{r=0}\not=0$, so in the limit of very small $r$ the STO-nG and the STO digress. 
