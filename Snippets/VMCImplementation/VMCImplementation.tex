...
\end{align}
where $A_i({\bf b})$ is the matix formed by replacing the $i$-th column of $A$ by ${\bf b}$ \cite{lay}\comment{p177}.} we can find an explicit expression for the matrix inverse in terms of the determinant as \cite{hjorth-jensen}\comment{p521}
\begin{align}
...
\end{align}
meaning the denominator drops out of \eq{VMCI2} and $R_\text{SD}$ simplifies immensely to \cite{hammond,ceperley,hjorth-jensen}\comment{p276}\comment{}\comment{p521}
\begin{align}
