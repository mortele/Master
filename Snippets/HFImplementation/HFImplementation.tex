...
\section{A comment on the density functional code}
Due to time constraints, the DFT code has not been properly tested and validated. Because of this, we leave the implementation specifics out of the present work and employ only the HF and VMC schemes for further calculations. 

The theory section is left in the thesis because the numerical integration part is an accessible introduction to the grids used in DFT for anyone wishing to implement their own density functional calculations from scratch. It is the author's wish that someone hopefully will find it useful at some point in the future.
